...
// Plot int in RAM starting at the current plot position, restoring the original plot position
void PlotInt(long a, int digits) {
  int count = digits;
  x0 += digits * 6;
  while (1) {
    int d = a % 10;
...
// Plot string in flash memory starting at the current plot position
...
void setup() {
  ...
  TCCR0A = 2<<COM0A0 | 2<<COM0B0 | 3<<WGM00; //32KHz PWM instead of 60Hz
  ...
  active = EEPROM.read(EEPROM.length() - 1);
  EEPROM.write(EEPROM.length() - 1, !active);
}

void loop() {
  // put your main code here, to run repeatedly:
  
  int Iset = constrain(map(analogRead(A0), 537, 977, 0, 3000), 0, 3000); //in mA
  int Inow = constrain(map(analogRead(A2), 0, 950, 0, 3000), 0, 3000); //in mA
  int Vnow = map(analogRead(A3), 0, 825, 0, 30000); //in mV
  long Pnow = (Inow * Vnow);
  
  if (Iset > Inow + 4) gateVoltage = constrain(gateVoltage + 1, 0, 2550);
  if (Iset > Inow + 200) gateVoltage = constrain(gateVoltage + 10, 0, 2550);
  if (Iset < Inow - 4) gateVoltage = constrain(gateVoltage - 1, 0, 2550);
  if (Iset < Inow - 200) gateVoltage = constrain(gateVoltage - 10, 0, 2550);
  if (active) analogWrite(1,gateVoltage/10);
  updateCount++;
  ...
  if (updateCount > 500){
    ...
    MoveTo(0,54);
    if (active) PlotText(PSTR("Enabled"));
    else PlotText(PSTR("Disabled"));

    MoveTo(0,30);
    PlotText(PSTR("Set:")); //Set reading ranges from 537-977
    PlotInt(Iset,4); 
    PlotText(PSTR("mA"));
